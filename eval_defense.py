...
def accuracy_calculator_for_combined_models_with_known_trigger(x, y, bd_model, tuned_pruned_model, trigger_labels):
    bd_result = np.argmax(bd_model.predict(x), axis=1)
    # print(bd_result)
    # print(bd_result.shape)
    tp_result = np.argmax(tuned_pruned_model.predict(x), axis=1) # tp for tuned_pruned
    ...
    for i, (bd, tp, trigger) in enumerate(zip(bd_result, tp_result,trigger_labels)):
        # print(i, (bd, tp)s)
    ...
    numof_bd  = sum(x==True for x in trigger_labels)
    # acc while clean 
...
if __name__ == '__main__':
    ...
    print("repaired net on its corresponding poisoned data: overall acc, inferred attack_ratio: ", 
        accuracy_calculator_for_combined_models(
        input_data_x, input_data_y, bd_model, tuned_pruned_model))

    if trigger_labels:
        print(" true attack_ratio:", sum(x==True for x in trigger_labels)/len(trigger_labels))
        ...
        print(
            accuracy_calculator_for_combined_models_with_known_trigger(
